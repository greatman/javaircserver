...
public class JoinCodec extends MessageCodec<JoinMessage>{

    public JoinCodec() {
        super(JoinMessage.class, "JOIN");
    }

    public String encode(JoinMessage message) throws IOException {
        return "JOIN " + message.getChannel();
    }

    public JoinMessage decode(String[] buffer) throws IOException {
        return new JoinMessage(buffer[0]);
    }
}
